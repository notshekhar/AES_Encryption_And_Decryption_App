...
class crypto extends Aes {
   ...
   *
   * @example
   *   const encr = crypto.encrypt('big secret', 'pāşšŵōřđ', 256); // 'lwGl66VVwVObKIr6of8HVqJr'
   */
  static encrypt(plaintext, password) {
    const nBits = 256;
    if (![128, 192, 256].includes(nBits)) throw new Error('Key size is not 128 / 192 / 256');
    plaintext = crypto.utf8Encode(String(plaintext));
    password = crypto.utf8Encode(String(password));
    ...
    // ------------ perform encryption ------------
    const ciphertextBytes = crypto.nistEncryption(plaintextBytes, key, counterBlock);
    ...
    const ciphertextB64 = crypto.base64Encode(nonceStr + ciphertextUtf8);
   ...
   *
   * @example
   *   const decr = crypto.decrypt('lwGl66VVwVObKIr6of8HVqJr', 'pāşšŵōřđ', 256); // 'big secret'
   */
  static decrypt(ciphertext, password) {
    const nBits = 256;
    if (![128, 192, 256].includes(nBits)) throw new Error('Key size is not 128 / 192 / 256');
    ciphertext = crypto.base64Decode(String(ciphertext));
    password = crypto.utf8Encode(String(password));
    ...
    // ------------ perform decryption ------------
    const plaintextBytes = crypto.nistDecryption(ciphertextBytes, key, counterBlock);
    ...
    const plaintext = crypto.utf8Decode(plaintextUtf8);
